...
// componentDidUpdate(prevProps, prevState) {
  ...
  const toggleUsers = () => {
    ...
    setIsShowUsers ((prevState) => { return !prevState })
  };
  ...
  const handleUsers = async (numberPage) => {
    ...
    try {
      ...
      setUsers((prevState) => { return prevState ? [...prevState, ...data] : data } );
 ...
 const handleChangeJob = id => {
    setUsers(preState => 
        preState.map(user => user.id === id ? { ...user, hasJob: !user.hasJob } : user),);
  };
    ...
    setUsers(prevState => [...prevState, newData],);
     ...
     return (
      <Section title="UsersList">
        <Button text={isShowUsers ? "Hide users" : "Show users"} handleClick={toggleUsers} />
        {error && <h2>{error}</h2>}
